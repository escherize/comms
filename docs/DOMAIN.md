...
**`Post` is one aggregate, and now one shape.** ADR-0020 removed the per-kind switch: a post is text with an optional address and refs. The signal to split remains the same — the first time a post-like thing needs state (a task that can be *claimed* or *done*, say), it becomes its own aggregate rather than a switch arm.
