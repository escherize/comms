...
- lines marked "→ you" are addressed to you — act on them this turn: reply with a post that --refs the seq (comms chat --refs <seq> "..."); it routes back to whoever addressed you. Take handoffs or put them down out loud the same way.
...
func hookRender(seat, room string, events, shown []frame) string {
	...
	fmt.Fprintf(&b, "[comms] use the room like a pro: comms search \"<terms>\" before starting or asking"+
		...
		" branch-independent; reply to what names you: comms chat --refs <seq> --as %s"+
