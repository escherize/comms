...
func skillCommands(t *testing.T, doc string) []skillCommand {
	...
	for i := 0; i < len(lines); i++ {
		...
		// and not ours to run.
		stdin := ""
		if idx := strings.Index(full, "| agent_comms"); idx != -1 {
		...
		if !strings.HasPrefix(full, "agent_comms") {
			continue
		}
...
func TestEverySkillCommandRuns(t *testing.T) {
	...
	for _, c := range cmds {
		...
		if code != ExitOK {
			t.Errorf("SKILL.md line %d: `agent_comms %s` exited %d\n%s",
...
func TestTheThreeSurfaceDocsAgreeOnTheVerbs(t *testing.T) {
	...
	if !strings.Contains(string(readme), "agent_comms enrol --as") {
		t.Error("the README's agent onboarding must be enrol")
	}

	// The skill teaches a subset, and every verb it teaches must exist.
	for _, m := range regexp.MustCompile(`agent_comms ([a-z]+)`).FindAllStringSubmatch(skill, -1) {
		verb := m[1]
		var known bool
		for _, v := range Verbs {
			if v == verb {
		...
		if !known {
			t.Errorf("SKILL.md teaches `agent_comms %s` and there is no such verb", verb)
		}
	}
}
