...
// reference it constantly" and asked for exactly this. Like version, the card
// itself is the output: prose on stdout, no terminal JSON object, because the
...
The room on one card: posting, addressing, exit codes, first moves. The full
contract is comms skill comms; this is the quick reference to keep hot.`)
	...
	b.WriteString(`posting — a post is text; there is no kind to choose:
  comms post "auth.py:88 flakes under -race #finding p2"
  mark what should be findable in the text (#finding, p2, a ticket id) —
  search is full-text. --step 3 --of 7 reports progress on a long job.

addressing — talking TO someone, not about them:
  a LEADING @seat in the text, or --to <seat>, addresses: the post breaks
  the ambient band and lands in front of that seat
  comms ask --to <seat> --text "..."      question; the reply routes back to you
...
must act now. Long content: comms attach <file|->, then post with --attach-hash.
`)
