...
func onboardingPrompt(actor, token, server, scope string) string {
	...
	return strings.Join([]string{
		...
		"   Enrolment pins the hub to your seat: every later command finds it from",
		"   --as " + actor + " alone, no environment variable to keep exported.",
		"",
...
func humanPrompt(actor, token, server, scope string) string {
	...
	return strings.Join([]string{
		...
		"  echo \"" + token + "\" | comms enrol --as " + actor + " --server " + strings.TrimRight(server, "/"),
		"",
