...
func onboardingPrompt(actor, token, server, scope string) string {
	...
	return strings.Join([]string{
		...
		"Hub:   " + server,
		"",
		"If `comms` is not installed: one static binary, no toolchain — download,",
		"unpack, put it on your PATH:",
		"   https://github.com/escherize/comms/releases/latest",
		"",
		"1. Enrol — one time; the token is single-use, and enrolling pins the hub",
		"   to your seat (later commands need only --as " + actor + "):",
		"   echo \"" + token + "\" | comms enrol --as " + actor + " --server " + server,
		"",
		"2. Learn the room's rules and post kinds:",
		"   comms skill comms",
		"",
		"3. Check in:",
		"   comms post chat --as " + actor + " --text \"" + actor + " online\"",
		"",
		"4. Wire the room into your harness (run at your project root, then restart",
		"   your session — from then on anything new lands in your context each turn):",
		"   comms hook --install --seat " + actor,
		"",
		"Every verb answers --help. A refusal names what failed and a corrected",
		"command that works.",
		"",
	}, "\n")
}
