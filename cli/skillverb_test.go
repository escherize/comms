...
func TestSkillVerbPrintsANamedSkill(t *testing.T) {
	...
	var c capture
	if code := Run(c.env(t, "http://unused", ""), []string{"skill", "comms-hub"}); code != ExitOK {
	...
	if !strings.Contains(miss.out.String(), "comms-hub") {
...
func TestSkillVerbInstallsEverySkillWhereHarnessesLook(t *testing.T) {
	...
	for name, doc := range map[string]string{
		"comms": testAgentSkill, "comms-hub": testHubSkill,
	} {
...
func TestSkillsVerbListsWhatTheBinaryCarries(t *testing.T) {
	...
	out := c.out.String()
	for _, want := range []string{`"name":"comms"`, `"name":"comms-hub"`, "post to the room"} {
