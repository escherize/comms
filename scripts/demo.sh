...
BIN="$WORK/comms"
export COMMS_HOME="$WORK/home"
export COMMS_SERVER="http://127.0.0.1:$PORT"
...
i=0
while [ $i -lt 50 ]; do
	curl -fsS -m 1 "$COMMS_SERVER/rooms" -H 'Accept: application/json' >/dev/null 2>&1 && break
	i=$((i + 1)); sleep 0.2
done
[ $i -lt 50 ] || { echo "demo: the hub never came up"; cat "$WORK/serve.log"; exit 1; }
echo "hub up on $COMMS_SERVER"
...
export COMMS_ACTOR=agent:bcm/claude-1
say() { printf '\n== %s\n' "$1"; }
...
# COMMS_RUN is what makes that true. Without it the scope is the process,
